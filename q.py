...
class Q:
    def __init__(self, params, agent_num, num_dizitized_pv_ratio, num_dizitized_soc, num_elastic_ratio_pattern):
        ...
        self.pv_sell_qtb_list = [np.full((pv_sell_rows, cols), 1000.0) for _ in range(self.agent_num)]
    ...
    def set_digitized_states(self, agent_id, pv_ratio, battery_soc, ev_battery_soc, elastic_ratio):
        ...
        return dr_state, battery_state, ev_battery_state, pv_states
    
    @property
    def get_states_(self):
        return self.dr_states, self.battery_states, self.ev_battery_states, self.pv_states
    
    def get_agent_states(self, agent_id):
        return self.dr_states[agent_id], self.battery_states[agent_id], self.ev_battery_states[agent_id], self.pv_states[agent_id]
    ...
    def set_actions(self, agent_id, episode, is_train):
        if is_train:
            ...
            if epsilon <= np.random.uniform(0, 1):
                ...
                reversed_battery_sell_qtb = self.battery_sell_qtb_list[agent_id][int(self.battery_states[agent_id])][::-1]
                max_index = len(self.battery_sell_qtb_list[agent_id][int(self.battery_states[agent_id])]) - 1 - np.argmax(reversed_battery_sell_qtb)
                ...
                reversed_ev_battery_sell_qtb = self.ev_battery_sell_qtb_list[agent_id][int(self.ev_battery_states[agent_id])][::-1]
                max_index = len(self.ev_battery_sell_qtb_list[agent_id][int(self.ev_battery_states[agent_id])]) - 1 - np.argmax(reversed_ev_battery_sell_qtb)
                next_action_list.append(max_index + int(self.params['price_min']))
                ...
                next_action_list.append(np.argmax(self.pv_sell_qtb_list[agent_id][int(self.pv_states[agent_id])]) + int(self.params['price_min']))
            else:
                for i in range(6):
    ...
    def update_q_table(self, agent_id, states, actions, rewards, next_states):
        ...
        pv_sell_td_error = rewards[5] + gamma * np.max(self.pv_sell_qtb_list[agent_id][next_states[5], :]) - self.pv_sell_qtb_list[agent_id][states[5], 
                                                                                                                              int(actions[5]-self.params['price_min'])]
...
if __name__ == '__main__':
    ...
    dr_states, battery_states, ev_battery_states, pv_states = q.get_states_
    print(dr_states)
    ...
    q.reset_all_actions()
    q.set_actions(agent_id=0, episode=1, is_train=True)
