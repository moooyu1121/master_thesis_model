...
class Preprocess:
    def __init__(self, seed=42) -> None:
        ...
        self.ev_charging_bool_df = pd.DataFrame()
        self.ev_move_consumption_df = pd.DataFrame()
        self.elastic_ratio_df = pd.DataFrame()
        self.seed = seed

    def set(self, demand_df:pd.DataFrame, 
            supply_df:pd.DataFrame, 
            price_df:pd.DataFrame, 
            ev_charging_bool_df:pd.DataFrame,
            ev_move_consumption:pd.DataFrame,
            elastic_ratio_df:pd.DataFrame):
        ...
        self.ev_charging_bool_df = ev_charging_bool_df
        self.ev_charging_bool_df['timestamp'] = pd.to_datetime(self.ev_charging_bool_df['timestamp'])
        self.ev_charging_bool_df.set_index('timestamp', inplace=True)
        self.ev_move_consumption = ev_move_consumption
        self.ev_move_consumption['timestamp'] = pd.to_datetime(self.ev_move_consumption['timestamp'])
        self.ev_move_consumption.set_index('timestamp', inplace=True)
        self.elastic_ratio_df = elastic_ratio_df
    ...
    def generate_car_charge(self, n):
        num_columns = self.ev_charging_bool_df.shape[1]
        columns = generate_random_integers(num_columns, n, seed=self.seed)
...
if __name__ == '__main__':
    ...
    print(d_df)
    print(s_df)
    car_charge_df, car_move_consumption_df = preprocess.generate_car_charge(20)
    print(car_charge_df)
    print(car_move_consumption_df)
